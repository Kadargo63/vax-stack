...
class Appointments extends Component {
    constructor(props) {
        ...
        this.viewCancelled = this.viewCancelled.bind(this);
    }
    ...
    handleActive(id) {
        Appointment.active(id).then((res) => {
            if (res.status === 200) {
                this.getAppointment();
                toast("Appointment Activated Succesfully.");
            }
        });
    }
    handleCancel(id) {
        Appointment.cancel(id).then((res) => {
            if (res.status === 200) {
                this.getAppointment();
                toast("Appointment Cancelled Succesfully.");
            }
        });
    }
    handleDelete(id) {
    ...
    getAppointment() {
        ...
        Appointment.getallappoints(params.status).then((res) => {
            let datas = [];
            res.data.forEach((element) => {
                data.push({ ...element, visible: false });
            });
            if (datas.length === 0) {
                this.setState({
                    showing: true,
